...
[samtools](https://github.com/samtools/samtools) 是處理高通量定序（Next-Generation Sequencing, NGS）比對資料的核心工具套件，用於操作 SAM（Sequence Alignment/Map）與 BAM（Binary Alignment/Map）文件格式。SAM/BAM 是儲存序列與參考基因組比對結果的標準格式，而 samtools 提供快速、高效、彈性的指令協助使用者解析與管理這些資料。
...
| **duplicate**（如果 BAM 有標記）          | PCR duplicates 數量                               | library PCR bias 或 over-amplification       | WGS 通常 <15%，RNA-seq 可自然偏高                      |
