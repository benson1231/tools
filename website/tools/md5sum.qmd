...
```bash
md5sum data/fastq/a.chr21_R1.fastq.gz
```

輸出格式如下，前半段為 md5 checksum，後半段為檔名：

```
d72fda8017c8eb035df69e3f5a3602bf  data/fastq/a.chr21_R1.fastq.gz
```
...
```bash
md5sum data/fastq/a.chr21_R1.fastq.gz > data/sample.md5
```
...
```bash
md5sum -c data/sample.md5
```

若檔案未被修改或損壞，會顯示：

```
data/fastq/a.chr21_R1.fastq.gz: OK
```
