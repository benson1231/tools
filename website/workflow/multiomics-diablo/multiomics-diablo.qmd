...
這些特徵是 DIABLO 在該 component 中選出的變數，例如：
...
loading 表示該特徵在 component 中的權重：

- **loading 越大** → 對 component 的影響越大  
- **loading 越小** → 對模型影響較小  

因此 loading 較大的特徵通常是模型中較重要的 biomarker。

3️⃣ **顏色代表不同 omics block**
...
每個 component 代表一組由多個特徵加權組合形成的 latent variable，因此 loading 較高的特徵通常與該 component 所代表的 biological signal 有較強的關聯。

因此，`plotLoadings()` 常用於 **篩選潛在 biomarker**，並作為後續功能分析（例如 pathway analysis 或 network analysis）的候選分子。
:::
...
1️⃣ **列 (rows) 代表特徵 (features)**  

這些特徵是 DIABLO 模型在指定 component 中選出的變數，例如：

- gene（mRNA）
- miRNA
- protein

不同顏色的區塊代表不同的 omics block。

2️⃣ **欄 (columns) 代表樣本 (samples)**  

每一欄代表一個樣本，通常會依據表現模式進行 **hierarchical clustering**。

3️⃣ **顏色代表表現量 (expression level)**  

熱圖的顏色表示特徵在不同樣本中的相對表現：
...
- 模型選出的特徵具有良好的分類能力  
- 這些 biomarker 能夠反映不同 biological states。

因此，`cimDiablo()` 常用於 **整體檢視 multi-omics biomarker signature 在樣本之間的表現模式**。
:::
