...
# vcftools <Position-Filtering> (位置過濾)
...
# vcftools <Variant-Type-Filtering>（變異類型過濾）
...
# vcftools <Filter-Flag-Filtering>（FILTER欄位過濾）
...
# vcftools <Info-Filtering>（INFO 欄位過濾）
...
# vcftools <Allele-Filtering>（等位基因過濾）
...
# vcftools <Genotype-Filtering>（基因型層級過濾）
...
# vcftools <Quality-Filtering>（品質過濾）

* **--minQ <float>**：只保留 QUAL 值 ≥ 指定 threshold 的位點。

---

# vcftools <Individual-Filtering>（樣本層級過濾）
...
## Ts/Tv Statistics（Transition/Transversion）
...
# vcftools <Comparison-Options>（VCF檔案比較）
